//
//  MineralizationBasedSigmoidG.swift
//  BalticBoxModel
//
//  Created by Bo Gustafsson on 2019-07-07.
//  Copyright © 2019 Bo Gustafsson. All rights reserved.
//

import Foundation

struct MineralizationBasedSigmoidG: GFunction {
    ...
    func g(p: Double, x: Double, c: Double, kappa: Double, m: Double, alpha: Double, r: Double, layer: Int) -> Double {
        if layer == 1 {
            return max(0.5 * (tanh(m) + tanh(q(x: x, c: c, kappa: kappa, m: m, alpha: 1.0, r: r))), 0.0)
        }
        return max(0.5 * (tanh(m) + tanh(q(x: x, c: c, kappa: kappa, m: m, alpha: alpha, r: r))), 0.0)
    }
    ...
    func dgdx(p: Double, x: Double,c: Double, kappa: Double, m: Double, alpha: Double, dalphadx: Double, r: Double, layer: Int) -> Double {
        ...
        let tanhq = tanh(q(x: x, c: c, kappa: kappa, m: m, alpha: alpha, r: r))
        return 0.5 * (1.0 - tanhq * tanhq) * dqdx(x: x, c: c, kappa: kappa, alpha: alpha, dalphadx: dalphadx, r: r)
    }
    func dgdc(p: Double, x: Double,c: Double, kappa: Double, m: Double, alpha: Double, r: Double, layer: Int) -> Double {
        if layer == 1 {
        let tanhq = tanh(q(x: x, c: c, kappa: kappa, m: m, alpha: 1.0, r: r))
        return 0.5 * (1.0 - tanhq * tanhq) * dqdc(x: x, c: c, kappa: kappa, alpha: 1.0, r: r)
        }
        let tanhq = tanh(q(x: x, c: c, kappa: kappa, m: m, alpha: alpha, r: r))
        return 0.5 * (1.0 - tanhq * tanhq) * dqdc(x: x, c: c, kappa: kappa, alpha: alpha, r: r)
   }

    private func q(x: Double, c: Double, kappa: Double, m: Double, alpha: Double, r: Double) -> Double {
        return kappa * x / (alpha * r * c) - m
    }
...
//    }
    ...
    func g(x01: Double, x0: Double, kappa: Double, m: Double, chi: Double, rho: Double, theta: Double, d: Double, alpha: Double, r: Double, layer: Int) -> Double {
        ...
        return max(0.5 * (tanh(m) + tanh(q(x01: x01, x0: x0, kappa: kappa, chi: chi, rho: rho, theta: theta, d: d, m: m, layer: layer))), 0.0)
    }
    ...
    private func q(x01: Double, x0: Double, kappa: Double, chi: Double, rho: Double, theta: Double, d: Double, m: Double, layer: Int) -> Double {
        if layer == 1 {
            return kappa * theta / rho / d * x01 / (chi - x0) - m
        }
        return kappa / d * x0 / (chi - x0) - m
    }
